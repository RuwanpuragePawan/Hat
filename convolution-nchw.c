    ...
    const xnn_operator_t convolution_op)
{
  ...
  if(convolution_op->packed_weights.offset == XNN_CACHE_NOT_FOUND) {
    ...
    // 1. Calculate the inner dimensions safely: (gic * kh * kw)
    size_t inner_term;
    if (!xnn_safe_mul(context->group_input_channels, context->kernel_height, &inner_term) ||
        !xnn_safe_mul(inner_term, context->kernel_width, &inner_term)) {
      return xnn_status_out_of_memory;
    }
    
    // 2. Safely add the bias term (+1)
    if (inner_term > SIZE_MAX - 1) {
      return xnn_status_out_of_memory;
    }
    inner_term += 1;

    // 3. Calculate total packed weights size across groups and output channels, including element size shift
    size_t packed_weights_size;
    if (!xnn_safe_mul(context->groups, packed_group_output_channels, &packed_weights_size) ||
        !xnn_safe_mul(packed_weights_size, inner_term, &packed_weights_size) ||
    ...
    const xnn_operator_t convolution_op)
{
  ...
  if(convolution_op->packed_weights.offset == XNN_CACHE_NOT_FOUND) {
    // 1. Calculate kernel dimensions safely
    size_t inner_term;
    if (!xnn_safe_mul(context->kernel_height, context->kernel_width, &inner_term)) {
      return xnn_status_out_of_memory;
    }
    ...
    // 3. Multiply by groups and shift for element size safely
    size_t packed_weights_size;
    if (!xnn_safe_mul(context->groups, inner_term, &packed_weights_size) ||
        !xnn_safe_mul(packed_weights_size, (size_t)1 << log2_filter_element_size, &packed_weights_size)) {
      return xnn_status_out_of_memory;
    }
    const size_t aligned_total_weights_size = round_up_po2(packed_weights_size, XNN_ALLOCATION_ALIGNMENT);
