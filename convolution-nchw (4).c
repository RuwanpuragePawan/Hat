    ...
    const xnn_operator_t convolution_op)
{
  ...
  if(convolution_op->packed_weights.offset == XNN_CACHE_NOT_FOUND) {
    ...
    // Calculate total packed weights size: groups * packed_group_output_channels * (group_input_channels * kernel_height * kernel_width + 1) * (1 << log2_filter_element_size)
    size_t inner_term;
    ...
    if (!xnn_safe_mul(context->group_input_channels, context->kernel_height, &inner_term) ||
        ...
        !xnn_safe_add(inner_term, 1, &inner_term) ||
        !xnn_safe_mul(context->groups, packed_group_output_channels, &packed_weights_size) ||
    ...
    const xnn_operator_t convolution_op)
{
  ...
  if(convolution_op->packed_weights.offset == XNN_CACHE_NOT_FOUND) {
    // Calculate total packed weights size: groups * (kernel_height * kernel_width + 1) * (1 << log2_filter_element_size)
    size_t inner_term;
    size_t packed_weights_size;
    if (!xnn_safe_mul(context->kernel_height, context->kernel_width, &inner_term) ||
        !xnn_safe_add(inner_term, 1, &inner_term) ||
        !xnn_safe_mul(context->groups, inner_term, &packed_weights_size) ||
        !xnn_safe_mul(packed_weights_size, (size_t)1 << log2_filter_element_size, &packed_weights_size)) {
      return xnn_status_out_of_memory;
    }
    const size_t aligned_total_weights_size = round_up_po2(packed_weights_size, XNN_ALLOCATION_ALIGNMENT);
